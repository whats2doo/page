<?php

$whats2dooIcon = 'logo-whats2doo';
$mailIcon = 'ios-mail';
$linkedinIcon = 'logo-linkedin';
$xingIcon = 'logo-xing';
$githubIcon = 'logo-github';

return [
    [
        'name'           => 'Ilja Leithold',
        'jobtitle'       => 'Chief Executive Officer',
        'text'           => 'ilja-leithold',
        'image'          => 'ilja-leithold.jpg',
        'socialprofiles' => [
            'whats2doo' => [
                'url'  => 'https://www.whats2doo.com/userprofile/r1DslZsYe',
                'icon' => $whats2dooIcon
            ],
            'linkedin'  => [
                'url'  => 'https://www.linkedin.com/in/ilja-leithold/',
                'icon' => $linkedinIcon
            ],
            'xing'      => [
                'url'  => 'https://www.xing.com/profile/Ilja_Leithold/',
                'icon' => $xingIcon
            ],
            'github'    => [
                'url'  => '',
                'icon' => $githubIcon
            ],
            'mail'      => [
                'url'  => '',
                'icon' => $mailIcon
            ]
        ]
    ],

    [
        'name'           => 'Hannes Rothwell',
        'jobtitle'       => 'Chief Operations Officer / Head of ICO',
        'text'           => 'hannes-rothwell',
        'image'          => 'hannes-rothwell.jpg',
        'socialprofiles' => [
            'whats2doo' => [
                'url'  => 'https://www.whats2doo.com/userprofile/S1mpNgjYx',
                'icon' => $whats2dooIcon
            ],
            'linkedin'  => [
                'url'  => 'https://www.linkedin.com/in/hannesrothwell/',
                'icon' => $linkedinIcon
            ],
            'xing'      => [
                'url'  => 'https://www.xing.com/profile/Hannes_Rothwell/cv',
                'icon' => $xingIcon
            ],
            'github'    => [
                'url'  => '',
                'icon' => $githubIcon
            ],
            'mail'      => [
                'url'  => '',
                'icon' => $mailIcon
            ]
        ]
    ],

    [
        'name'           => 'Yanis Hamdali',
        'jobtitle'       => 'Chief Marketing Officer',
        'text'           => 'yanis-hamdali',
        'image'          => 'yanis-hamdali.jpg',
        'socialprofiles' => [
            'whats2doo' => [
                'url'  => 'https://www.whats2doo.com/userprofile/Syht5MGib',
                'icon' => $whats2dooIcon
            ],
            'linkedin'  => [
                'url'  => 'https://www.linkedin.com/in/yanishamdali/',
                'icon' => $linkedinIcon
            ],
            'xing'      => [
                'url'  => 'https://www.xing.com/profile/Yanis_Hamdali/cv',
                'icon' => $xingIcon
            ],
            'github'    => [
                'url'  => '',
                'icon' => $githubIcon
            ],
            'mail'      => [
                'url'  => '',
                'icon' => $mailIcon
            ]
        ]
    ],

    [
        'name'           => 'Lucas Löffel',
        'jobtitle'       => 'Chief Technology Officer',
        'text'           => 'lucas-loeffel',
        'image'          => 'lucas-loeffel.jpg',
        'socialprofiles' => [
            'whats2doo' => [
                'url'  => '',
                'icon' => $whats2dooIcon
            ],
            'linkedin'  => [
                'url'  => 'https://www.linkedin.com/in/lucas-l%C3%B6ffel-919458103/',
                'icon' => $linkedinIcon
            ],
            'xing'      => [
                'url'  => 'https://www.xing.com/profile/Lucas_Loeffel2/cv',
                'icon' => $xingIcon
            ],
            'github'    => [
                'url'  => 'https://github.com/loeffel-io',
                'icon' => $githubIcon
            ],
            'mail'      => [
                'url'  => '',
                'icon' => $mailIcon
            ]
        ]
    ],

    [
        'name'           => 'Julian Daiber',
        'jobtitle'       => 'Chief Web Officer',
        'text'           => 'julian-daiber',
        'image'          => 'julian-daiber.jpg',
        'socialprofiles' => [
            'whats2doo' => [
                'url'  => 'https://www.whats2doo.com/userprofile/S1JyfUd9x',
                'icon' => $whats2dooIcon
            ],
            'linkedin'  => [
                'url'  => '',
                'icon' => $linkedinIcon
            ],
            'xing'      => [
                'url'  => 'https://www.xing.com/profile/Julian_Daiber/cv',
                'icon' => $xingIcon
            ],
            'github'    => [
                'url'  => 'https://github.com/kstar20001',
                'icon' => $githubIcon
            ],
            'mail'      => [
                'url'  => '',
                'icon' => $mailIcon
            ]
        ]
    ],

    [
        'name'           => 'Patrick Rujan',
        'jobtitle'       => 'Marketing Manager',
        'text'           => 'patrick-rujan',
        'image'          => 'patrick-rujan.jpg',
        'socialprofiles' => [
            'whats2doo' => [
                'url'  => 'https://www.whats2doo.com/userprofile/HyMiTaoax',
                'icon' => $whats2dooIcon
            ],
            'linkedin'  => [
                'url'  => 'https://www.linkedin.com/in/patrick-rujan-a76a54b9/',
                'icon' => $linkedinIcon
            ],
            'xing'      => [
                'url'  => 'https://www.xing.com/profile/Patrick_Rujan/cv',
                'icon' => $xingIcon
            ],
            'github'    => [
                'url'  => '',
                'icon' => $githubIcon
            ],
            'mail'      => [
                'url'  => '',
                'icon' => $mailIcon
            ]
        ]
    ],

    [
        'name'           => 'Jacqueline Odendahl',
        'jobtitle'       => 'Accounting and Controlling Manager',
        'text'           => 'jacqueline-odendahl',
        'image'          => 'jacqueline-odendahl.jpg',
        'socialprofiles' => [
            'whats2doo' => [
                'url'  => '',
                'icon' => $whats2dooIcon
            ],
            'linkedin'  => [
                'url'  => 'https://www.linkedin.com/in/jacqueline-odendahl-68074493/',
                'icon' => $linkedinIcon
            ],
            'xing'      => [
                'url'  => 'https://www.xing.com/profile/Jacqueline_Odendahl/cv',
                'icon' => $xingIcon
            ],
            'github'    => [
                'url'  => '',
                'icon' => $githubIcon
            ],
            'mail'      => [
                'url'  => '',
                'icon' => $mailIcon
            ]
        ]
    ],

    [
        'name'           => 'Kristina Drenker',
        'jobtitle'       => 'Manager Communication Design & PR',
        'text'           => 'kristina-drenker',
        'image'          => 'kristina-drenker.jpg',
        'socialprofiles' => [
            'whats2doo' => [
                'url'  => '',
                'icon' => $whats2dooIcon
            ],
            'linkedin'  => [
                'url'  => 'https://www.linkedin.com/in/kristinadrenker/',
                'icon' => $linkedinIcon
            ],
            'xing'      => [
                'url'  => 'https://www.xing.com/profile/KristinaLisa_Drenker/cv',
                'icon' => $xingIcon
            ],
            'github'    => [
                'url'  => '',
                'icon' => $githubIcon
            ],
            'mail'      => [
                'url'  => '',
                'icon' => $mailIcon
            ]
        ]
    ],

    [
        'name'           => 'Paul Sprotte',
        'jobtitle'       => 'Chief Backend Developer',
        'text'           => 'paul-sprotte',
        'image'          => 'paul-sprotte.jpg',
        'socialprofiles' => [
            'whats2doo' => [
                'url'  => '',
                'icon' => $whats2dooIcon
            ],
            'linkedin'  => [
                'url'  => '',
                'icon' => $linkedinIcon
            ],
            'xing'      => [
                'url'  => '',
                'icon' => $xingIcon
            ],
            'github'    => [
                'url'  => '',
                'icon' => $githubIcon
            ],
            'mail'      => [
                'url'  => '',
                'icon' => $mailIcon
            ]
        ]
    ],

    [
        'name'           => 'Maximilian Tolksdorf',
        'jobtitle'       => 'Community Manager',
        'text'           => 'maximilian-tolksdorf',
        'image'          => 'maximilian-tolksdorf.jpg',
        'socialprofiles' => [
            'whats2doo' => [
                'url'  => '',
                'icon' => $whats2dooIcon
            ],
            'linkedin'  => [
                'url'  => '',
                'icon' => $linkedinIcon
            ],
            'xing'      => [
                'url'  => '',
                'icon' => $xingIcon
            ],
            'github'    => [
                'url'  => '',
                'icon' => $githubIcon
            ],
            'mail'      => [
                'url'  => '',
                'icon' => $mailIcon
            ]
        ]
    ],

    [
        'name'           => 'Cake',
        'jobtitle'       => 'Happiness Manager',
        'text'           => 'cake',
        'image'          => 'cake.jpg',
        'socialprofiles' => [
            'whats2doo' => [
                'url'  => '',
                'icon' => $whats2dooIcon
            ],
            'linkedin'  => [
                'url'  => '',
                'icon' => $linkedinIcon
            ],
            'xing'      => [
                'url'  => '',
                'icon' => $xingIcon
            ],
            'github'    => [
                'url'  => '',
                'icon' => $githubIcon
            ],
            'mail'      => [
                'url'  => '',
                'icon' => $mailIcon
            ]
        ]
    ]
];